...
def n_star_inference(n_stars,iteration,elem_err=False,n_init=20000,n_samples=1000,max_stars=100):    
    ...
    lowBound = pt._shared(np.asarray([-5,std_log_SFR_crit,-5,std_min_time]))
    upBound = pt._shared(np.asarray([5,5,5,std_max_time]))
    ...
    w0=pt._shared(w_array_0)
    b0=pt._shared(b_array_0)
    w1=pt._shared(w_array_1)
    b1=pt._shared(b_array_1)
    ...
    with simple_model:
        ...
        TimeSq = pt.reshape(Locals[:,-1]**2.,(n_stars,1))
